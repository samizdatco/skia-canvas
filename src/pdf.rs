...
pub fn open(mut cx: FunctionContext) -> JsResult<JsValue> {
  ...
  // check for the %PDF magic number so the caller can distinguish pdfs from other image formats
  if is_pdf(buffer.as_slice(&cx)){
    let data = Data::new_copy(buffer.as_slice(&cx));
    // a pdf that won't parse yields an empty page list, indistinguishable from a zero-page document
    let pages = read_document(&data).unwrap_or_default();
