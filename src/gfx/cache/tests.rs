...
use crate::gfx::page::{ExportOptions, Page, PageStamp};

use super::*;
use super::budget::{charge, Entry, LIVE_BYTES};
use super::rasters::{EmbedKey, Snapshot};
use super::readback;
...
fn put_embed(store:&mut Store, page:usize, bytes:u64, age_ms:u64, reads:u32){
  let mut entry = Entry::new(image(), bytes, reads, false);
  entry.last_read = Instant::now() - Duration::from_millis(age_ms);
  charge(&entry);
  store.embeds.map.insert(EmbedKey{ page, placement:0, epoch:0, depth:1 }, entry);
}

fn has_export(store:&Store, page:usize) -> bool{ store.exports.map.contains_key(&(page, 0)) }
fn has_embed(store:&Store, page:usize) -> bool{
  store.embeds.map.contains_key(&EmbedKey{ page, placement:0, epoch:0, depth:1 })
}
fn count(store:&Store) -> usize{ store.exports.map.len() + store.embeds.map.len() }
...
fn eviction_prefers_reuse_density_over_size_and_kind(){
  ...
  // backwards: a big export snapshot nothing has read against a small embed raster serving
  ...
  put_embed(&mut store, 2, 1*MB, 0, 200);

  store.sweep_within(8*MB);
  assert!(has_embed(&store, 2), "a raster serving every frame was evicted to keep an unread snapshot");
...
fn a_zero_byte_marker_is_never_a_budget_victim(){
  ...
  put_embed(&mut store, 2, 4*MB, 0, 0);
...
fn the_ledger_spans_stores_and_is_credited_on_drop(){
  ...
  put_embed(&mut resident, 1, 6*MB, 0, 0);
  {
    let mut window = Store::new(Residency::Window);
    put_embed(&mut window, 2, 6*MB, 0, 0);
...
fn only_a_live_raster_vetoes_retirement(){
  ...
  // …a raster does
  put_embed(&mut device.0.borrow_mut(), 2, 4*MB, 0, 0);
...
fn a_sweep_takes_victims_in_rank_order_across_all_three_kinds(){
  ...
  put_embed (&mut store, 2, 4*MB, 2000, 4);           // b: density 1, stale
  put_embed (&mut store, 3, 4*MB, 0, 4);              // c: density 1, fresh
  ...
  assert!(!has_embed(&store, 2), "of two entries tied on reuse density, the staler goes");
  assert!(has_embed(&store, 3), "…and the fresher survives");
...
fn the_shared_store_is_process_wide_and_device_stores_are_not(){
  let _lock = serialized();
  let stamp = PageStamp{ id: 9, epoch: 0, depth: 1 };
  ...
  // The same claim for embeds, asked of the map directly rather than through `Cache::embed` —
  ...
  let key = EmbedKey::new(stamp.id, 0, stamp.epoch, stamp.depth);
  let holds = |cache:Cache| cache.with(|store| store.embeds.read(key, dims).is_some());
  let seed = |cache:Cache| cache.with(|store| store.embeds.put(stamp, 0, image(), MB));
