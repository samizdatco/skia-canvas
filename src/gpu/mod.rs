...
// rayon's workers have no autorelease pool of their own, so Metal's objc allocations would
// accumulate until the thread exits. the main thread gets one from node's event loop.
#[cfg(feature = "metal")]
