...
// a bad property name (here a digit-leading ident) no longer poisons the rest of the list
tokenize!(recover_bad_name, "9x: red; fill: blue", declare("fill", "blue"));
...
// skia-canvas: parse nested parens in values so they aren't truncated at the first `)`. Upstream
// skipped a function body to the first `)` only.

tokenize!(
    nested_calc,
    "width: calc((100% - 20px) / 2)",
    declare("width", "calc((100% - 20px) / 2)")
);

tokenize!(
    nested_var_fallback,
    "fill: var(--accent, rgb(0,0,0))",
    declare("fill", "var(--accent, rgb(0,0,0))")
);

tokenize!(
    nested_gradient,
    "background: linear-gradient(to right, rgba(0,0,0,.5), rgba(255,255,255,.5))",
    declare(
        "background",
        "linear-gradient(to right, rgba(0,0,0,.5), rgba(255,255,255,.5))"
    )
);

// a `)` inside a quoted argument no longer closes the function early
tokenize!(
    nested_paren_in_string,
    "background: url(\"a)b\")",
    declare("background", "url(\"a)b\")")
);

// a nested-paren value doesn't corrupt the declaration after it
tokenize!(
    nested_then_valid,
    "width: calc((1px)); fill: blue",
    declare("width", "calc((1px))"),
    declare("fill", "blue")
);

// skia-canvas: parse custom-property declarations (`--foo`) instead of tripping on the name. We
// don't interpret them — they're captured and passed along like any other declaration.

tokenize!(custom_prop_basic, "--accent: #f00", declare("--accent", "#f00"));

tokenize!(custom_prop_hyphenated, "--main-color: blue", declare("--main-color", "blue"));

tokenize!(custom_prop_var, "--x: var(--y, 1px)", declare("--x", "var(--y, 1px)"));

// a custom property mid-list is kept, and the declarations around it survive
tokenize!(
    custom_prop_kept,
    "--a: 1; fill: blue",
    declare("--a", "1"),
