...
class App extends RustClass{
  ...
  get windows(){ return [..._windows] }
  get running(){ return this.#running }
  get fps(){ return this.#fps }
  set fps(rate){
    if (rate > 0 && rate != this.#fps){
      this.#fps = this.ƒ('setRate', rate)
    }
  }

  launch(){
    if (this.#running) throw new Error("Application is already running")
    this.#running = true

    // begin event loop (and never return)
    this.ƒ("launch", args => {
      let {frame, changes, state} = JSON.parse(args)

      // update local state based on ui modifications
      _windows.forEach(win => {
        Object.assign(win, (state || {})[win.state.id])
      })

      // deliver ui events to corresponding windows
      for (const [id, events] of Object.entries(changes || {})){
        _windows.filter(w => w.state.id==id).forEach(win => {
          let modifiers = {}
          for (const [[type, e]] of events.map(o => Object.entries(o))){
            switch(type){
              ...
              case 'mouse':
                ...
                e.events.forEach(type => win.emit(type, {type, x, y, pageX, pageY, button, ...modifiers}))
              break

              case 'input':
                win.emit(type, {type, value:e, code:e.charCodeAt(), ...modifiers})
              break

              case 'keyboard':
                let {event, key, code, repeat} = e
                win.emit(event, {type:event, key, code, repeat, ...modifiers})
              break
      ...
      // trigger the `frame` event (and erase the previous content as a side effect)
      _windows.filter(win => win.listenerCount('frame')).forEach(win => {
      ...
      // update the display
      return [
        JSON.stringify( _windows.map(win => win.state) ),
        _windows.map(win => core(win.canvas.pages[win.page-1]) )
      ]
    })
  }
...
class Window extends EventEmitter{
  static #kwargs = "left,top,width,height,title,page,background,fullscreen,fps,frame,cursor,fit,visible".split(/,/)
  #canvas
  #state

  constructor(canvas={}, opts={}){
    // accept either ƒ(canvas, {…}) or ƒ({…})
    [canvas, opts] = (canvas instanceof Canvas) ? [canvas, opts]
                   : [new Canvas(canvas.width || 300, canvas.height || 150), canvas]
    canvas.getContext("2d") // ensure the context has at least one page
    ...
    Object.assign(this.#state, Object.fromEntries(
      Object.entries(opts).filter(([k, v]) => Window.#kwargs.includes(k) && v!==undefined)
    ))

    Object.assign(this, Object.fromEntries(
      Object.entries(this.#state).filter(([k, v]) => Window.#kwargs.includes(k))
    ))

    _windows.push(this)
    neon.App.openWindow(JSON.stringify(this.#state), core(canvas.pages[this.page-1]))
  }
  ...
  set title(txt){ this.#state.title = (txt ?? '').toString() }

  get cursor(){ return this.#state.cursor }
  set cursor(icon){ if (css.cursor(icon)) this.#state.cursor = icon }
  ...
  set page(val){ if (this.#canvas.pages[val-1]) this.#state.page = val }

  get background(){ return this.#state.background }
  set background(c){ this.#state.background = (c ?? '').toString() }

  emit(){
    // report errors in event-handlers but don't crash
    try{ super.emit(...arguments) }
    catch(err){ console.error(err) }
  }

  close(){
    _windows = _windows.filter(w => w !== this)
    neon.App.closeWindow(this.state.id)
  }
}
...
module.exports = {
  ...
  FontLibrary: new FontLibrary(), App: new App()
}
