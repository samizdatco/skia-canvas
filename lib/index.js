...
class Canvas extends RustClass{
  ...
  async saveAsync(filename, {format, quality=100}={}){
    var seq
    filename = filename.replace(/{(\d*)}/g, (_, pad) => {
      pad = parseInt(pad, 10)
      seq = isFinite(pad) ? pad : isFinite(seq) ? seq : -1
    ...
    let worker = new EventEmitter(),
        ...
        contexts = Canvas.contexts.get(this).map(core)
    this.ƒ("saveAsync", filename, seq, fmt, quality, callback, contexts)
  ...
  toBufferAsync(extension, {format="png", quality=100, page}={}){
    ({format, quality, page} = Object.assign(
      {format, quality, page},
      typeof extension == 'string' ? {format:extension}
    : typeof extension == 'object' ? extension
    ...
    let worker = new EventEmitter(),
        callback = (result, msg) => worker.emit(result, msg),
        contexts = Canvas.contexts.get(this),
        pages = isFinite(idx) ? contexts.slice(idx, idx+1)
              : fmt=='pdf' ? contexts
              : contexts.slice(0, 1)

    this.ƒ("toBufferAsync", fmt, quality, callback, pages.map(core))
    return new Promise((res, rej) => worker.once('ok', res).once('err', msg => rej(new Error(msg))) )
  }
