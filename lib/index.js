...
class App extends RustClass{
  ...
  launch(){
    ...
    this.ƒ("launch", args => {
      let {frame, changes} = JSON.parse(args)
      ...
      let windows = _windows.filter(win => win.listenerCount('frame')),
          specs = windows.map(win => win.state),
          contexts = windows.map(win => {
            win.canvas.width = win.canvas.width
            win.emit("frame", frame)
            return core(win.canvas.getContext("2d"))
          })

      return [JSON.stringify(specs), contexts]
    })
  }
...
class Window extends EventEmitter{
  ...
  constructor(canvas={}, opts={}){
    ...
    _windows.push(this)
    neon.App.openWindow(JSON.stringify(this.#state), core(canvas.getContext("2d")))
  }
