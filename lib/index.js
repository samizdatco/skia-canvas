...
class Canvas extends RustClass{
  ...
  get engine(){ return this.prop('engine') }
  set engine(mode){ this.prop('engine', toString(mode)) }
  ...
  [REPR](depth, options) {
    let {width, height, async, engine, pages} = this
    return `Canvas ${inspect({width, height, async, engine, pages}, options)}`
  }
}
