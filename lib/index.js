...
const ø = Symbol.for('📦'), // the attr containing the boxed struct
      ...
      inject = (obj, struct) => hidden(obj, ø, struct), // set the struct
      native = Object.entries(require('./v6')).reduce( (api, [name, fn]) => {
...
class RustClass{
  constructor(type){
    hidden(this, 'native', native[type.name])
  }
  ...
  init(fn, ...args){
    return inject(this, this.native[fn](null, ...args))
  }
...
const hidden = (obj, attr, value) => (
  Object.defineProperty(obj, attr, {value, writable:true, enumerable:false})
)
...
class Path2D extends RustClass{
  ...
  // boolean operations
  complement(path){ return inject(new Path2D(), this.ƒ("op", core(path), "complement")) }
  difference(path){ return inject(new Path2D(), this.ƒ("op", core(path), "difference")) }
  intersect(path){  return inject(new Path2D(), this.ƒ("op", core(path), "intersect")) }
  union(path){      return inject(new Path2D(), this.ƒ("op", core(path), "union")) }
  xor(path){        return inject(new Path2D(), this.ƒ("op", core(path), "xor")) }

  // elide overlaps
  simplify(){       return inject(new Path2D(), this.ƒ('simplify')) }
}
