...
class Canvas extends RustClass{
  ...
  saveAs(filename, opts={}){
    ...
    let {format, quality, pages, padding, pattern, density, fonts} = parse.output(this.pages, {filename, ...opts}),
        args = [pages.map(core), pattern, padding, format, quality, density, fonts];
  ...
  toBuffer(extension="png", opts={}){
    opts = typeof opts=='number' ? {quality:opts} : opts
    let {format, quality, pages, density, fonts} = parse.output(this.pages, {extension, ...opts}),
