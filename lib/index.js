...
class CanvasRenderingContext2D extends RustClass{
  ...
  getImageData(x, y, width, height, {colorType='rgba', colorSpace='srgb'}={}){
    ...
    h = Math.floor(height),
    buffer = this.ƒ('getImageData', x, y, w, h, {colorType, colorSpace});
...
class ImageData{
  constructor(...args){
    ...
    readOnly(this, "data", data || new Uint8ClampedArray(width * height * io.bytesPerPixel(colorType)))
  }
...
function fetchData(src, ok, fail, mode='sync'){
  if (Buffer.isBuffer(src)) {
    ...
    try{ ok({data:readFileSync(src), src}) }
    catch(e){ fail(e) }
  } else {
