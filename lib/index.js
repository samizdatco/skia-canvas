...
class Canvas extends RustClass{
  ...
  saveAs(filename, opts={}){
    ...
    let contexts = Canvas.contexts.get(this).map(core),
        {format, quality, pages, padding, pattern, density, fonts} = parse.output(contexts, {filename, ...opts}),
        args = [pages, pattern, padding, format, quality, density, fonts];
  ...
  toBuffer(extension="png", opts={}){
    opts = typeof opts=='number' ? {quality:opts} : opts
    let contexts = Canvas.contexts.get(this).map(core),
        {format, quality, pages, density, fonts} = parse.output(contexts, {extension, ...opts}),
        args = [pages, format, quality, density, fonts];
  ...
  toDataURL(extension="png", opts={}){
    ...
    let contexts = Canvas.contexts.get(this),
        {mime} = parse.output(contexts, {extension, ...opts}),
