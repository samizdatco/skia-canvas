var fs = require('fs'),
    ...
    {CanvasRenderingContext2D, CanvasGradient, CanvasPattern, Path2D, Image} = require('../native'),
    {extend, toColor, fromColor, toSkMatrix, fromSkMatrix, loadImage} = require('./utils');
...
extend(CanvasRenderingContext2D, {
  ...
  getImageData:function(...args){
    return new ImageData( _(this, 'getImageData', ...args) )
  }
...
// CanvasPattern
//
extend(CanvasPattern, {
  setTransform:function(matrix){
    if (arguments.length>1) matrix = [...arguments]
    _(this, 'setTransform', toSkMatrix(matrix) )
  }
})
      ...
      if (Buffer.isBuffer(src)) data = src
        ...
        get.concat(src, (err, res, data) => {
          ...
          if (err) onerror(err)
            ...
            if (_(this, "set_data", data)) onload(this)
...
class ImageData extends extend(native.ImageData){
...
module.exports = {CanvasRenderingContext2D, Path2D, Image, ImageData, DOMMatrix, DOMRect, DOMPoint, loadImage}
