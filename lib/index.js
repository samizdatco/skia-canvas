...
class App extends RustClass{
  ...
  launch(){
    ...
    let contexts = _windows.map(win => core(win.canvas.getContext("2d"))),
        specs = _windows.map(win => win.state)

    // begin event loop (and never return)
    this.#running = true
    this.ƒ("launch", JSON.stringify(specs), contexts, args => {
      for (let [message, info] of Object.entries(JSON.parse(args))){

        switch (message){
          case "frame":
            let windows = _windows.filter(win => win.listenerCount('frame')),
                window_ids = windows.map(win => win.state.id),
                contexts = windows.map(win => {
                  win.canvas.width = win.canvas.width
                  try{ win.emit("frame", info) }
                  catch(e){ console.error(e) }
                  return core(win.canvas.getContext("2d"))
                })
            return [JSON.stringify(window_ids), contexts]
        }
...
module.exports = {
  ...
  FontLibrary: new FontLibrary(), App:APP
}
