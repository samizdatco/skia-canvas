...
const crate = require('./v6'),
      ...
      {parseFont, parseVariant, parseFilter, parseExport} = require('./parse')
...
class Canvas extends RustClass{
  ...
  saveAs(filename, opts={}){
    ...
    let contexts = Canvas.contexts.get(this).map(core),
        {format, quality, padding, pattern, density, outline} = parseExport(contexts, opts)

    if (this.async){
      let worker = new EventEmitter(),
          callback = (result, msg) => worker.emit(result, msg)
      this.ƒ("save", pattern, padding, format, quality, callback, contexts)
      return new Promise((res, rej) => worker.once('ok', res).once('err', msg => rej(new Error(msg))) )
    }else{
      this.ƒ("saveSync", pattern, padding, format, quality, contexts)
    }
  }
  ...
  toBuffer(extension="png", opts={}){
    ...
    let contexts = Canvas.contexts.get(this),
        {format, quality, page, density, outline} = parseExport(contexts, opts),
    ...
    if (this.async){
      let worker = new EventEmitter(),
          callback = (result, msg) => worker.emit(result, msg)
      this.ƒ("toBuffer", format, quality, callback, pages.map(core))
      return new Promise((res, rej) => worker.once('ok', res).once('err', msg => rej(new Error(msg))) )
    }else{
      return this.ƒ("toBufferSync", format, quality, pages.map(core))
    }
  }
  ...
  toDataURL(extension="png", opts={}){
    opts.extension = extension
    let contexts = Canvas.contexts.get(this),
        {format, mime, quality, density, outline} = parseExport(contexts, opts)
