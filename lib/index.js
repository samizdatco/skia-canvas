...
class Canvas extends RustClass{
  ...
  saveAs(filename, {format, quality=100}={}){
    ...
    if (this.async){
      ...
      this.ƒ("save", filename, seq, fmt, quality, callback, contexts)
      return new Promise((res, rej) => worker.once('ok', res).once('err', msg => rej(new Error(msg))) )
    }else{
      this.ƒ("saveSync", filename, seq, fmt, quality, contexts)
    }
  }
  ...
  toBuffer(extension, {format="png", quality=100, page}={}){
    ...
    if (this.async){
      let worker = new EventEmitter(),
          callback = (result, msg) => worker.emit(result, msg)
      this.ƒ("toBuffer", fmt, quality, callback, pages.map(core))
      return new Promise((res, rej) => worker.once('ok', res).once('err', msg => rej(new Error(msg))) )
    }else{
      return this.ƒ("toBufferSync", fmt, quality, pages.map(core))
    }
  }
