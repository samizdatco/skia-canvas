...
class Image extends RustClass {
    ...
    const request = this.#fetch = {}, // use an empty object as a unique token
          loaded = ({data, src}) => {
            if (request === this.#fetch){ // confirm this is the most recent request with ===
              ...
              this.prop("src", src)
              if (this.prop("data", data)) this.emit('load', this)
              else this.emit('error', this.#err = new Error("Could not decode image data"))
            }
          },
          failed = (err) => {
            this.#fetch = undefined
  ...
  static fetchData(src, ok, fail, fsMode='sync'){
  ...
  constructor(...args){
    if (args[0] instanceof ImageData){
      ...
      var [data, width, height, {colorSpace='srgb', colorType='rgba'}={}] = args,
          bytesPerPixel = io.bytesPerPixel(colorType) // validates the string as side effect
...
const loadImageData = (src, ...args) => new Promise((res,rej) =>
  Image.fetchData(src, ({data}) => res(new ImageData(data, ...args)), err => rej(err), 'async')
)
