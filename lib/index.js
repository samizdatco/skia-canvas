...
class Canvas extends RustClass{
  ...
  saveAs(filename, opts={}){
    ...
    let {format, quality, pages, padding, pattern, density, outline, matte, msaa} = io.options(this.pages, {filename, ...opts}),
        args = [pages.map(core), pattern, padding, {format, quality, density, outline, matte, msaa}]
  ...
  saveAsSync(filename, opts={}){
    opts = typeof opts=='number' ? {quality:opts} : opts
    let {format, quality, pages, padding, pattern, density, outline, matte, msaa} = io.options(this.pages, {filename, ...opts})
    this.ƒ("saveSync", pages.map(core), pattern, padding, {format, quality, density, outline, matte, msaa})
  }

  toBuffer(extension="png", opts={}){
    opts = typeof opts=='number' ? {quality:opts} : opts
    let {format, quality, pages, density, outline, matte, msaa} = io.options(this.pages, {extension, ...opts}),
        args = [pages.map(core), {format, quality, density, outline, matte, msaa}];
  ...
  toBufferSync(extension="png", opts={}){
    opts = typeof opts=='number' ? {quality:opts} : opts
    let {format, quality, pages, density, outline, matte, msaa} = io.options(this.pages, {extension, ...opts})
    return this.ƒ("toBufferSync", pages.map(core), {format, quality, density, outline, matte, msaa})
  }
...
class ImageData{
  static #COLOR_SPACES = ["srgb"] // TODO: add display-p3 when supported…
  static #COLOR_TYPES = ["rgba", "rgb", "bgra", "argb"]

  constructor(...args){
    if (args[0] instanceof ImageData){
      var {data, width, height, colorSpace, colorType} = args[0]
    ...
    readOnly(this, "data", data || new Uint8ClampedArray(width * height * colorType.length))
  }

  [REPR](depth, options) {
    let {width, height, colorType, data} = this
    return `ImageData ${inspect({width, height, colorType, data}, options)}`
  }
}
...
const loadImageData = (src, ...args) => new Promise((res,rej) =>
  fetchData(src, ({data}) => res(new ImageData(data, ...args)), err => rej(err), 'async')
)


function fetchData(src, ok, fail, mode='sync'){
  if (Buffer.isBuffer(src)) {
    ...
    // local file path (synchronous)
    try{
      ok({data:readFileSync(src), src})
    }catch(e){
      fail(e)
    }
  } else {
    // local file path (async)
...
module.exports = {
  ...
  TextMetrics, Image, ImageData, Path2D, loadImage, loadImageData, ...geometry,
