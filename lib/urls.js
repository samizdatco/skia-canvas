...
// http clients & proxy agents will be initialized lazily
let http, https, HttpsProxyAgent
...
const fetchURL = (url, opts, ok, fail) => {
  if (!http) ({http, https} = require('follow-redirects'))
  ...
  if (!client){
    ...
    if (opts.agent===undefined && PROXY_URL){
