...
function options(pages, {filename='', extension='', format, page, quality, density, outline, matte}={}){
  ...
  return {filename, pattern, format, mime, pages, padding, quality, density, outline, matte}
}
...
class Zip{
  ...
  toBuffer(){
    ...
    for (var {offset, name, header} of this.files){
...
const asDownload = async (canvas, mime, quality, filename) => {
  _download(filename, await asBlob(canvas, mime, quality))
}

const asZipDownload = (pages, mime, quality, pattern, padding) => {
  let filenames = i => pattern.replace('{}', String(i+1).padStart(padding, '0')),
      archive = 'canvas',
      zip = new Zip(archive)

  return Promise.all(contexts.map(async (page, i) => {
    let filename = filenames(i) // serialize filename(s) before awaiting
    await zip.add(filename, await asBlob(page, mime, quality))
  })).then( () =>
