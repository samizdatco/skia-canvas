...
const asDataURL = (canvas, mime, quality) => Promise.resolve(
  (canvas[Symbol.for("toDataURL")] || canvas.toDataURL)(mime, quality)
)
...
const asZipDownload = async (contexts, mime, quality, pattern, padding) => {
  ...
  for (let c2d of contexts){
    let blob = await asBlob(c2d.canvas, mime, quality),
        filename = pattern.replace('{}', String(idx++).padStart(padding, '0'))
...
if (isWeb){
  window.io = {asBuffer, asDataURL, asDownload, asZipDownload, options}
}else{
