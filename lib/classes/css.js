  ...
  if (cache.font[str]===undefined){
    try{
      ...
      let font = {style:'normal', variant:'normal', weight:'normal', stretch:'normal'},
          seen = {style:false, variant:false, weight:false, stretch:false},
      ...
      while (token = tokens.shift()) {
        ...
        switch (match){
          ...
          case "weight":
            if (token === 'normal'){
              let slot = ['style', 'variant', 'weight', 'stretch'].find(key => !seen[key])
              if (!slot){
                throw new Error(`Duplicate font attribute "${token}"`)
              }
              font[slot] = token
              seen[slot] = true
              break;
