...
const styleRE = /^(normal|italic|oblique)$/,
      ...
      axisTagRE = /^[\x20-\x7e]{4}$/, // opentype 4-char tag (in printable-ascii range)
...
function parseVariationSettings(str){
  ...
  // decompose css list into a {tag:value, …} dictionary
  let axes = new Map()
  for (let entry of splitBy(str, ',')){
    let parts = splitBy(entry.trim(), ' ')
    if (parts.length != 2) return null // reject the whole string if any entry is malformed

    let [rawTag, rawValue] = parts,
        tag = unquote(rawTag),
        value = Number(rawValue)
    if (rawTag == tag || !axisTagRE.test(tag)) return null
