...
class App extends RustClass{
  ...
  #running = false // whether the `eventLoop` property is permanently set
  #active = false // whether launch() can be called to start/resume the event loop
  #launcher // timer set by opening windows to ensure app is launched soon after
  #frameTimer // the screen-update timer for `node` events mode
  ...
  set eventLoop(mode){
    if (this.#running) throw new Error("Cannot alter event loop after it has begun")
  ...
  #render(roundtrip){
    ...
    if (active || this.eventLoop=='node'){
      ...
      this.#frameTimer = setTimeout(() => this.#render(roundtrip))
      if (this.eventLoop=='node' && !active) this.#frameTimer.unref()
    }else{
  ...
  #dispatch({ui, state, geom}){
    ...
    if (state) this.pruneWindows(win =>
      win.top === undefined || // keep new windows still waiting for a `geom` roundtrip
      win.id in state  // evict old windows that have been closed
    ).map(win => {
      // update local state based on gui interactions
    ...
    let events = this.#windows.map(win => win.eventNames()).flat(),
        needsFrameUpdates = (events.includes('frame') || events.includes('draw')),
        frameRate = needsFrameUpdates ? this.#fps : 0
    this.ƒ('setRate', frameRate)
  }

  getWindow(id, callback){
    this.#windows.filter(win => win.id==id).forEach(callback)
  }
  ...
  openWindow(win){
    ...
    else if (this.#frameTimer) this.#frameTimer.ref() // stay alive until window can open
    this.ƒ("openWindow", JSON.stringify(win.state), core(win.canvas.pages[win.state.page-1]))
  }

  closeWindow(win){
    this.pruneWindows(w => w!==win)
  ...
  pruneWindows(keep){
    this.#windows
      .filter(w => !keep(w))
      .map(win => this.emit('close', win))
    return this.#windows = this.#windows.filter(keep)
  }
...
class Window extends EventEmitter{
  ...
  close(){ GlobalApp.closeWindow(this) }
...
module.exports = {App:GlobalApp, Window}
