...
class App extends RustClass{
  ...
  dispatch({ui, state, geom}){
    // merge autogenerated window locations with the specs for newly opened windows
  ...
  launch(){
    ...
    const renderFrame = () => {
      let start = new Date()
      let dt = 1000/this.#fps
      let running = this.ƒ('activate', roundtrip)
      let elapsed = new Date() - start
      if (running) setTimeout(renderFrame, Math.max(0, dt - elapsed))
    }
