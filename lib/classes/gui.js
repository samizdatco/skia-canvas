...
class App extends RustClass{
  ...
  launch(){
    ...
    if (this.#active){
      ...
      this.ƒ('setRate', this.#fps)
      this.#active = true
      this.#render((isRender, args) => {
        this.#dispatch(isRender, JSON.parse(args))
        return isRender && [
  ...
  #dispatch(isRender, {geom, state, ui}){
    ...
    if (isRender) for (let win of this.#windows){
