...
class App extends RustClass{
  ...
  #frameTimer // timer for redraws & event-routing in `node` events mode
  ...
  #eachWindow(updates, callback){
    if (updates) for (const [id, payload] of Object.entries(updates)){
  ...
  #dispatch({geom, state, ui}){
    ...
    if (state) this.#windows = this.#windows.filter(win => {
      ...
      // but otherwise evict all windows that have been closed
      this.emit('close', win)
    })
