...
class App extends RustClass{
  ...
  constructor(){
    ...
    this.ƒ("register", this.#dispatch.bind(this))
  ...
  launch(){
    ...
    this.#started = true
    if (this.#active){
      return console.error('Application is already running')
    }
    this.#active = true

    return this.ƒ('activate').finally(() => {
      this.#active = false
      this.#launcher = null
    })
  ...
  #dispatch(isFrame, payload){
    ...
    if (ui) this.#eachWindow(ui, (win, events) => {
      for (const [[type, e]] of events.map(o => Object.entries(o))){
        switch(type){
          case 'mouse':
            var {button, point, page_point:{x:pageX, y:pageY}, modifiers} = e
            win.emit(e.event, {button, ...point, pageX, pageY, ...modifiers})
          break
