...
class CanvasRenderingContext2D extends RustClass{
  ...
  // per-page accessors (the canvas's size properties only align with the last page)
  get width(){ return this.prop('size')[0] }
  get height(){ return this.prop('size')[1] }

  getContextAttributes(){
    // captured when the page was created & fixed for the life of the context (see Canvas.newPage)
    let colorSpace = this.ref('colorSpace') ?? 'srgb'
    let willReadFrequently = this.ref('willReadFrequently') ?? false
