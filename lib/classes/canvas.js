...
const {RustClass, core, inspect, argc, REPR} = require('./neon'),
...
class Canvas extends RustClass{
  ...
  saveAs(filename, opts={}){
    ...
    let {pages, padding, pattern, ...rest} = exportOptions(this.pages, {filename, ...opts}),
  ...
  toBuffer(extension="png", opts={}){
    opts = typeof opts=='number' ? {quality:opts} : opts
    let {pages, ...rest} = exportOptions(this.pages, {extension, ...opts})
...
function exportOptions(pages, {filename='', extension='', format, page, quality, matte, density, msaa, fonts, outline, textContrast, textGamma, colorType}={}){
  ...
  if (outline!==undefined){
    console.warn("The `outline` option has been deprecated: use {fonts:\"outline\"} instead")
    if (outline && fonts===undefined) fonts = "outline"
  }
  ...
  return {filename, pattern, format, mime, pages, padding, quality, matte, density, msaa, fonts, textContrast, textGamma, colorType}
}
