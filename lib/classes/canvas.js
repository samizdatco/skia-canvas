...
function exportOptions(canvas, {filename='', extension=''}, opts){
  ...
  let {page, quality, matte, density, msaa, outline, downsample, colorType, colorSpace} = opts
  ...
  if (colorSpace===undefined){
    // default to the first context's color space
    let spaces = new Set(pages.map(page => page.ref('colorSpace')))
    colorSpace = spaces.size==1 ? [...spaces][0] : canvas.ref('colorSpace')
  }else{
  ...
  return {
    ...
    msaa, outline, textContrast, textGamma, downsample, colorType, colorSpace
  }
}
