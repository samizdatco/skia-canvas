...
const {fileURLToPath} = require('url'),
      ...
      {Image, ImageData, pixelSize, validColorSpace, getSharp} = require('./imagery'),
      {Path2D} = require('./path'),
...
class Canvas extends RustClass{
  ...
  getContext(kind, settings){
    ...
    // the colorSpace is sticky once selected on the first context created
    let colorSpace = this.ref('colorSpace') ?? settings?.colorSpace ?? 'srgb'
    this.ref('colorSpace', validColorSpace(colorSpace))

    // willReadFrequently is captured when the first context is created and inherited by every page
    if (this.ref('willReadFrequently') === undefined){
      this.ref('willReadFrequently', !!settings?.willReadFrequently)
    }
    return this.#contexts[0] || this.newPage()
  }
  ...
  newPage(width, height){
    ...
    if (this.ref('willReadFrequently') === undefined){
      this.ref('willReadFrequently', false) // can only be set via getContext
    }
...
class CanvasPattern extends RustClass{
  constructor(canvas, src, repeat){
    ...
    if (src instanceof Image){
      if (!src.complete) throw new Error("Image has not completed loading: listen for `load` event or await `decode()` first")
...
function exportOptions(canvas, {filename='', extension=''}, opts){
  ...
  // inherit the canvas-level default unless overridden
  colorSpace = colorSpace!==undefined ? validColorSpace(colorSpace) : canvas.ref('colorSpace')
