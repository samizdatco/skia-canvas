...
function loadNative(){
  ...
  if (require('fs').existsSync(local)) return require(local)

  // otherwise try the @skia-canvas/<platform-specific> package (for most other installs)
  let attempt = load => {
    try{ return load() }
    catch(e){
      if (e.code == 'MODULE_NOT_FOUND') throw new Error(nativeModuleNotFound()) // package not installed
  ...
  throw new Error(nativeModuleNotFound())
}
...
function nativeModuleNotFound(){
  ...
  return [
    ...
    `dependency but is not present. Some likely causes:`,
    ...
    `   (see https://github.com/npm/cli/issues/4828): delete node_modules and the lockfile,`,
    `   then re-run the install on this machine`,
    ` • node_modules was copied over from a machine with a different OS or architecture:`,
    `   re-install on this machine instead of syncing node_modules`,
