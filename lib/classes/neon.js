...
// retire the gpu context deterministically at process exit, rather than leaving it to a
// thread-local destructor during native shutdown
if (neon.gfx && typeof neon.gfx.retireGPU === 'function'){
  process.once('exit', () => { try{ neon.gfx.retireGPU() }catch{} })
}
...
function loadNative(){
  ...
  let attempt = load => {
    try{ return load() }
      ...
      if (e.code == 'MODULE_NOT_FOUND') throw new Error(nativeModuleNotFound(dlopenError)) // package not installed
