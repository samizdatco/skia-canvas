...
const loadImage = (src, args) => new Promise((res, rej) => {
  ...
  fetchData(src, options,
    (data, src, raw) => {
      let img = new Image()
      img.prop('src', src)
      if (img.prop('data', data, raw, pageNum)) res(img)
  ...
  set src(src){
    ...
    const loaded = (data, imgSrc, raw) => {
      if (request === this.#fetch){ // confirm this is the most recent request with ===
        ...
        if (this.prop("data", data, raw)){
          this.ref('error', null)
          this.emit('load', this)
        }else{
          this.#fail(DecodingError())
        }
      }
    }
    const failed = (err) => {
...
module.exports = {Image, ImageData, loadImage, loadImageData, validPixelSize, validColorSpace, getSharp, validImage}
