...
class Image extends RustClass {
  ...
  dispose(){
    ...
    // abandon an in-flight load so a pending decode() rejects rather than hanging forever.
    // only emit the event if there are listeners since otherwise it will throw
    if (this.#fetch){
      this.#fetch = undefined
      this.#err = DisposalError()
      if (this.listenerCount('error')) this.emit('error', this.#err)
    }
  ...
  // release state for this image then yield until the next event loop tick so other deferred
  // finalizers can be run. Can also be triggered via `await using`
  async release(){
    this.dispose()
...
module.exports = {Image, ImageData, loadImage, loadImageData, pixelSize, validColorSpace, getSharp}
