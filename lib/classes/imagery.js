...
const loadImageData = (src, ...args) => new Promise((res,rej) =>
  Image.fetchData(src, ({data}) => res(new ImageData(data, ...args)), err => rej(err))
)
...
class Image extends RustClass {
  ...
  set src(src){
    ...
    Image.fetchData(src, loaded, failed)
  }

  static fetchData(src, ok, fail){
    if (Buffer.isBuffer(src)) {
      ...
      get.concat(src, (err, res, data) => {
        ...
        if (err) fail(err)
          ...
          fail(new Error(`Failed to load image from "${src}" (error ${code})`))
        } else {
          ok({data, src})
        }
      })
...
module.exports = {Image, ImageData, loadImage, loadImageData}
