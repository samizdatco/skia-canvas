...
const _safeFetchURLs = ['http:', 'https:', 'data:', 'blob:']

const loadImageData = (src, width, height, settings) => {
  let {protocol} = new URL(src, window.location.href)
  if (!_safeFetchURLs.includes(protocol)){
    return Promise.reject(new Error(`Refusing to load image data from "${protocol}" URL`))
  }
  return fetch(src)
