...
class RustClass{
  ...
  ƒ(fn, ...args){
    let native = crate[`${this.constructor.name}_${fn}`]
    return native(this[ø], ...args);
  }
}
...
class CanvasRenderingContext2D extends RustClass{
  ...
  get imageSmoothingEnabled(){ this.ƒ("get_imageSmoothingEnabled")}
  set imageSmoothingEnabled(flag){ this.ƒ("set_imageSmoothingEnabled", flag)}

  get imageSmoothingQuality(){ this.ƒ("get_imageSmoothingQuality")}
