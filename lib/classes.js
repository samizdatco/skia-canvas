var fs = require('fs'),
    ...
    {toColor, fromColor} = require('./colors'),
...
const CanvasRenderingContext2D = RustClass(native.CanvasRenderingContext2D, {
  ...
  strokeStyle:{
    get:function(){
      return fromColor( $(this, 'get_strokeStyle') )
    },
    set:function(clr){
      let rgba = toColor(clr)
      if (rgba) $(this, 'set_strokeStyle', ...rgba )
    }
  },
  fillStyle:{
    get:function(){
      return fromColor( $(this, 'get_fillStyle') )
    },
    set:function(clr){
      let rgba = toColor(clr)
      if (rgba) $(this, 'set_fillStyle', ...rgba )
  }
  },
  shadowColor:{
    get:function(){
      return fromColor( $(this, 'get_shadowColor') )
    },
    set:function(clr){
      let rgba = toColor(clr)
      if (rgba) $(this, 'set_shadowColor', ...rgba )
    }
  },
...
const CanvasGradient = RustClass(native.CanvasGradient, {
  addColorStop:function(pct, clr){
    let rgba = toColor(clr)
    if (rgba) $(this, 'addColorStop', pct, ...rgba )
