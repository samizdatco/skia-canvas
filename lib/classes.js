...
class Canvas extends RustClass(native.Canvas){
  ...
  saveAs(filename, {format, quality=100}={}){
    ...
    let ext = (format || extname(filename).replace(/^\./, '')).toLowerCase()
    if (!ext.match(/png|jpe?g|pdf|svg|gif|heic|webp/)){
      throw new Error(`Unsupported file format "${ext}" (expected "png", "jpg", "pdf", or "svg")`)
    }

    $(this, "saveAs", filename, seq, ext, quality)
  }

  toBuffer(extension, {format="png", quality=100}={}){
    format = typeof extension == 'string' ? extension
           : typeof extension == 'object' ? extension.format
           : format;
    quality = typeof extension == 'object' ? extension.quality
            : quality

    if (!format.match(/png|jpe?g|pdf|svg|gif|heic|webp/i)){
      throw new Error(`Unsupported file format "${format}" (expected "png", "jpg", "pdf", or "svg")`)
    }

    return $(this, "toBuffer", format.toLowerCase(), quality)
  }
...
class CanvasRenderingContext2D extends RustClass(native.CanvasRenderingContext2D){
  ...
  measureText(...args){
    let [metrics, ...lines] = $(this, 'measureText', ...args)
