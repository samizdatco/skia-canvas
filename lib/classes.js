...
class Path2D extends RustClass(native.Path2D){
  ...
  // boolean operations
  difference(path){ return new Path2D($(this, "op", path, "difference")) }
  intersect(path){ return new Path2D($(this, "op", path, "intersect")) }
  union(path){ return new Path2D($(this, "op", path, "union")) }
  xor(path){ return new Path2D($(this, "op", path, "xor")) }
  complement(path){ return $(this, "op", path, "complement") }

  // remove overlaps
  simplify(path){ return $(this, "simplify") }
}
