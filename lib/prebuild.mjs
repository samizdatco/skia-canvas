...
// `gh release view` suddenly stopped including a `digest` field for assets (16 aug 25)
// for now switch to manually downloading and hashing release assets...
async function snapshot(){
    let {version} = await config(),
        json = (await exec(`gh release view v${version} --json assets`)).stdout,
        {assets} = JSON.parse(json)

    let hashes = await Promise.all(assets.map(({name, url}) => {
      let agent = PROXY_URL ? new HttpsProxyAgent(proxy) : undefined
      return fetch(url, {agent})
        .then(resp => {
          if (!resp.ok) throw Error(`Failed to retrieve binary "${name}" (HTTP error ${resp.status})`)
          return resp.bytes()
        }).then(bytes => {
          let sha = crypto.createHash('sha256')
          sha.update(bytes)
          return {name, digest:`sha256:${sha.digest('hex')}`}
        })
    }))
    exec(`npm pkg set prebuild='${JSON.stringify(hashes)}' --json`)
}
...
async function download(...args){
    ...
    try{
        ...
        if (!ok){
            ...
            console.log(`Fetching prebuilt libary from "${url}"`)
        }
...
function compile(...args){
  let optimization = args.includes('generic') || args.includes('dev') ? '' : "--release",
      features = args.includes('generic') ? '' : `--features ${CARGO_FEATURES}`,
