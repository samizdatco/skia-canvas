    ...
    try{
        let body = await new Promise((res, rej) => {
          https.get(url, {agent}, resp => {
            ...
            if (status == 404) rej(`Prebuilt library not found at "${url}" (HTTP error ${status})`)
            else if (status < 200 || status >= 300) rej(`Failed to load prebuilt binary from "${url}" (HTTP error ${status})`)
            else res(resp)
          })
        })
