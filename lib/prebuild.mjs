...
// features to be passed to cargo
const FEATURES = {
  darwin: "metal,window",
...
async function upload(_, version, triplet){
...
async function download(opts, version, triplet, prebuild){
    ...
    try{
        ...
        if (!opts.includes('--or-build') || !existsSync(`${ROOT}/Cargo.toml`)) process.exit(1)
...
function compile(opts=[]){
  let features = opts.includes('generic') ? '' : `--features ${FEATURES}`,
      mode = opts.length ? '' : "--release"

  console.warn(`cargo build ${[mode, features].filter(s=>s).join(' ')}`)
  child_process.spawnSync(
    `cargo-cp-artifact -nc ${BINARY_PATH} -- cargo build --message-format=json-render-diagnostics ${mode} ${features}`,
...
async function main(){
    ...
    await ({upload, download, snapshot, compile}[cmd] || usage)(opts, version, triplet, prebuild)
}
main()
