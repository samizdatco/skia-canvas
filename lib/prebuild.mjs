...
async function config(){
  let {family} = await import('detect-libc'),
      package_json = JSON.parse(await readFile(PACKAGE_JSON)),
...
async function missing(){
    let {version} = JSON.parse(await readFile(PACKAGE_JSON))

    try{
        // compare the platforms list to the release's current contents and report what's missing
        let {assets} = JSON.parse((await exec(`gh release view v${version} --json assets`)).stdout),
            present = new Set(assets.map(({name}) => name)),
            absent = PLATFORMS.filter(p => !present.has(`${p.triplet}.gz`))

        console.log(JSON.stringify(Object.fromEntries(
            [...new Set(PLATFORMS.map(p => p.os))].map(os => [os,
                absent.filter(p => p.os == os).map(({cpu, libc}) => libc ? {arch:cpu, libc} : {arch:cpu})
            ])
        ), null, 2))
    }catch(e){
        console.error(`Could not read the v${version} release: ${e.message}`)
        process.exit(1)
    }
}
...
async function download(...args){
    ...
    let https = (await import('follow-redirects/https.js')).default,
        {HttpsProxyAgent} = await import('https-proxy-agent'),
        {version, triplet, prebuild} = await config(),
...
async function usage(){
    ...
    console.log(`    missing - list the platforms with no <triplet>.gz on the v${version} release,`)
...
async function main(){
    ...
    await ({upload, download, collect, compile, packages, missing}[cmd] || usage)(...args)
}
main()
