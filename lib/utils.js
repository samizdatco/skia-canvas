...
function fromColor(components){
  if (!Array.isArray(components) || components.length<4) return null

  let alpha = components.slice().pop()
  let [r,g,b,...rest] = components.map(c => Math.floor(c*255))
  return alpha == 1 ? `#${convertColor.rgb.hex(r, g, b).toLowerCase()}`
       : `rgba(${r},${g},${b},${alpha.toFixed(3).replace(/0*$/, '').replace(/\.$/, '')})`
