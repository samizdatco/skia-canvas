...
function parseOutput(pages, {filename='', extension, format, page, quality, density, fonts}={}){
  ...
  if (isFinite(idx) && idx < 0 || idx >= pp) throw new RangeError(
    pp == 0 ? `Canvas has no associated contexts (try calling getContext or newPage first)` :
    pp == 1 ? `Canvas only has a ‘page 1’ (${page} is out of bounds)`
            : `Canvas has pages 1–${pp} (${page} is out of bounds)`
  )

  pages = isFinite(idx) ? pages.slice(idx, idx+1)
  ...
  return {filename, pattern, format, mime, pages, padding, quality, density, fonts}
}
