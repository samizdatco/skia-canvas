...
function parseExport(pages, {filename='', extension, format, page, quality, density, outline}={}){
  ...
  if (quality===undefined){
    ...
    if (format != 'jpg'){
      throw new TypeError(`The quality option is only valid for jpeg images (not ${format})`)
    }else if (typeof quality!='number' || !Number.isInteger(quality) || quality<0 || quality>100){
      throw new TypeError("The quality option must be an integer in the 0–100 range")
    }
  }
  ...
  if (outline===undefined){
    ...
    if (format != 'svg'){
      throw new TypeError(`The outline option is only valid for fonts in svg images (not ${format})`)
    }else{
      outline = !!outline
    }
  }
  ...
  return {filename, pattern, format, mime, page:idx, padding, quality, density, outline}
}


module.exports = {parseFont, parseVariant, parseSize, parseFilter, parseExport}