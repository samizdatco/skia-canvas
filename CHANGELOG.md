...
- The bitmap generated by [getImageData()][mdn_getImageData] is now cached. When called repeatedly, only newly added drawing commands will need to be rasterized (and will be layered atop the bitmap saved in the prior call).
...
- Argument validation now emulates browser behavior much more closely—including converting what were previously TypeErrors in certain cases into silent failures. To reenable these errors, set the `SKIA_CANVAS_STRICT` environment variable to something truth-y.
