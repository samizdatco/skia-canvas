...
- Added initial SVG rendering support. **Image**s can now load SVG files and can be drawn in a resolution-independent manner via [`drawImage()`][mdn_drawImage]. Caveats & quirks include:
  - **Image**s loaded from SVG files that don't have a `width` and `height` set on their root `<svg>` element will report a `naturalWidth` and `naturalHeight` of `undefined`. When drawn, these size-less Images will be scaled to fit within the **Canvas**'s current bounds (and will behave as if their natural size is this scaled-to-fit size for the purposes of an 8-argument `drawImage` call).
