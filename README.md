...
| --                           | --                            | --                                               |
| [**width**][canvas_width]    | [**engine**][canvas_engine] ⚡| ~~[**async**][canvas_async]~~  ⚡                    |
...
The `.engine` attribute allows you to control whether rendering occurs on the graphics card or uses the CPU. By default, rendering is hardware accelerated, using OpenGL on macOS and Vulkan on Linux and Windows. If you wish to switch to software-based rendering, set the `.engine` property to the string `"cpu"`. To resume GPU-based rendering, set it to `"gl"` on the mac or `"vulcan"` on linux/windows.
